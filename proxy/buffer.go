...
const (
	PG_MESSAGE_LENGTH_OFFSET_STARTUP int = 0
	PG_MESSAGE_LENGTH_OFFSET         int = 1
)

/*
 * A PGMessage is a variable-sized byte buffer used to read and write
 * PostgreSQL Frontend and Backend messages.
 *
 * A separate instance of a PGMessage should be use for reading and writing.
 */
...
/*
 * NewPGMessage creates and intializes a new PGMessage using message as its
 * initial contents.
 */
func NewPGMessage(message []byte) *PGMessage {
	return &PGMessage{
 ...
 * returned.
 */
func (message *PGMessage) ReadInt32() (int32, error) {
	value := make([]byte, 4)
 ...
 * returned.
 */
func (message *PGMessage) ReadInt16() (int16, error) {
	value := make([]byte, 2)
 ...
 * buffer.
 */
func (message *PGMessage) ReadByte() (byte, error) {
 ...
 */
func (message *PGMessage) ReadBytes(count int) ([]byte, error) {
 ...
 * message buffer.
 */
func (message *PGMessage) ReadString() (string, error) {
 ...
 */
func (message *PGMessage) WriteByte(value byte) error {
 ...
 */
func (message *PGMessage) WriteBytes(value []byte) (int, error) {
 ...
 */
func (message *PGMessage) WriteInt16(value int16) (int, error) {
	x := make([]byte, 2)
 ...
 */
func (message *PGMessage) WriteInt32(value int32) (int, error) {
	x := make([]byte, 4)
 ...
 */
func (message *PGMessage) WriteString(value string) (int, error) {
 ...
 *
 * offset should be one of the PG_MESSAGE_LENGTH_OFFSET* constants.
 */
func (message *PGMessage) ResetLength(offset int) {
	...
	/* Calculate the new length and set it. */
	binary.BigEndian.PutUint32(s, uint32(len(s)))
}

/*
 * Message gets the contents of the message buffer. This function is only
 * useful after 'Write' operations as the underlying implementation will return
 * the 'unread' portion of the buffer.
 *
 * The slice that is returned by this function is only valid until another call
 * modifying call is made to the buffer (Write, Read, etc.).
 */
func (message *PGMessage) Message() []byte {
