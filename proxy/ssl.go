...
func upgradeServerConnection(client net.Conn) net.Conn {
	if config.Cfg.Credentials.SSL.Enable {
		...
		cert, _ := tls.LoadX509KeyPair(
			config.Cfg.Credentials.SSL.SSLServerCert,
			config.Cfg.Credentials.SSL.SSLServerKey)

		glog.Infoln(config.Cfg.Credentials.SSL.SSLServerCert)
 ...
 *
 */
func upgradeClientConnection(node *config.Node, connection net.Conn) net.Conn {
	verifyCA := false
	 ...
	 * error.
	 */
	switch config.Cfg.Credentials.SSL.SSLMode {
	...
	case SSL_MODE_REQUIRE:
		 ...
		 */
		verifyCA = (config.Cfg.Credentials.SSL.SSLRootCA != "")
	...
	case SSL_MODE_DISABLE:
		return connection
	default:
		glog.Fatalf("Unsupported sslmode %s\n",
			config.Cfg.Credentials.SSL.SSLMode)
	}

	/* Add client SSL certificate and key. */
	glog.V(2).Infoln("Loading SSL certificate and key.")
	cert, _ := tls.LoadX509KeyPair(
		config.Cfg.Credentials.SSL.SSLCert,
		config.Cfg.Credentials.SSL.SSLKey)
	tlsConfig.Certificates = []tls.Certificate{cert}

	/* Add root CA certificate. */
	glog.V(2).Infoln("Loading root CA.")
	tlsConfig.RootCAs = x509.NewCertPool()
	rootCA, _ := ioutil.ReadFile(config.Cfg.Credentials.SSL.SSLRootCA)
