...
At this stage of development, *crunchy-proxy* performance
is promising, currently for example it takes roughly 14 milliseconds on
a cheap development box (e.g. i7 Core 6700K, SSD)
to process 100 very small SQL statements (e.g. /* read */ select now()).
